  ...
  if (!$username || !$password || !$nome_salone) {
    ...
    if ($chk->num_rows > 0) {
      ...
      try {
        ...
        // ── 4. Crea utente amministratore collegato al salone ───────────────────────────────
        ...
        // ── 5. Collega utente e salone con ruolo di proprietario ───────────────────────────────
        $conn->query("INSERT INTO user_saloni (user_id, salone_id, ruolo)
                              VALUES ($user_id, $salone_id, 'proprietario')");

        // ── 6. Impostazioni predefinite per il nuovo salone ─────────────────────────────────────────────
  ...
  <style>
    ...
    *::after {
    ...
    body::before {
      ...
      position: fixed;
      inset: 0;
      background: radial-gradient(ellipse 60% 50% at 15% 50%, rgba(154, 121, 232, .1) 0%, transparent 70%);
      pointer-events: none
    }
    ...
    .logo-icon {
      ...
      justify-content: center;
      font-size: 1.4rem;
      box-shadow: 0 8px 20px rgba(154, 121, 232, .25);
      margin-bottom: 10px
    }
    ...
    .field input:focus {
      ...
      outline: none;
      box-shadow: 0 0 0 3px rgba(154, 121, 232, .12)
    }
...
<body>
  <div class="wrap">
    <div class="logo">
      ...
      <div style="font-family:'Cormorant Garamond',serif;font-weight:300;font-size:1.4rem;color:var(--text)">Gestionale
        Salone</div>
    </div>
    <div class="card">
      ...
      <?php if ($successo): ?>
        <div class="ok">✓ <?php echo htmlspecialchars($successo) ?> <a href="login.php"
            style="color:#86efac;font-weight:600">accedi ora →</a></div><?php endif ?>
      <?php if (!$successo): ?>
        <form method="POST">
          <div class="field">
            <label>nome del salone *</label>
            <input type="text" name="nome_salone" placeholder="es. Lumière Beauty" required autofocus
              value="<?php echo htmlspecialchars($_POST['nome_salone'] ?? '') ?>">
            <small>verrà usato anche come indirizzo pubblico prenotazioni</small>
          </div>
          <div class="divider"></div>
          <div class="field">
            <label>username amministratore *</label>
            <input type="text" name="username" placeholder="es. admin_lumiere" required
