...
<div class="griglia-4">
  ...
  <div class="card">
    ...
    <div class="barra-prog" style="margin-top:8px">
      <div class="barra-prog-fill" style="width:<?php echo $tasso_occ ?>%"></div>
    </div>
  </div>
  <div class="card">
...
<div class="griglia-2">
  <div class="card">
    ...
    <?php else: ?>
      ...
      <?php foreach ($incassi_mensili as $im): ?>
        <?php [$y, $m2] = explode('-', $im['periodo']);
        $pct = round($im['totale'] / $max_inc * 100); ?>
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
          <div style="width:70px;font-size:.8rem;color:var(--text-muted);flex-shrink:0"><?php echo $nomi_mesi[(int) $m2] ?>
            <?php echo $y ?></div>
          <div class="barra-prog" style="flex:1">
            <div class="barra-prog-fill" style="width:<?php echo $pct ?>%;background:var(--accent2)"></div>
          </div>
          <div style="width:70px;text-align:right;font-size:.82rem;font-weight:600">
            €<?php echo number_format($im['totale'], 0, ',', '.') ?></div>
        </div>
      <?php endforeach ?>
    <?php endif ?>
  </div>
  ...
  <div class="card">
    <h3 style="font-size:1rem;color:var(--text);margin-bottom:16px">appuntamenti per giorno —
      <?php echo $nomi_mesi[$mese] ?> <?php echo $anno ?></h3>
    <?php if (!$per_giorno): ?>
    ...
    <?php else: ?>
      ...
      <?php foreach ($per_giorno as $g): ?>
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:7px">
          <div style="width:32px;font-size:.8rem;color:var(--text-muted);flex-shrink:0;text-align:right">
            <?php echo $g['giorno'] ?></div>
          <div class="barra-prog" style="flex:1">
            <div class="barra-prog-fill" style="width:<?php echo round($g['n'] / $max_g * 100) ?>%"></div>
          </div>
...
<?php if ($top_servizi): ?>
  <div class="card">
    <h3 style="font-size:1rem;color:var(--text);margin-bottom:16px">top servizi — <?php echo $nomi_mesi[$mese] ?>
      <?php echo $anno ?></h3>
    <table>
      <thead>
        <tr>
