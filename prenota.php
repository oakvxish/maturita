...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$nome || !$cognome || !$svid || !$data_raw) {
    ...
    // Verifica se il servizio appartiene al salone
  ...
  <div class="wrap">
    ...
    <?php if ($successo): ?>
      ...
      <div class="conferma">
        ...
        <div class="dettagli">
          <div style="font-size:.65rem;letter-spacing:.3em;text-transform:uppercase;color:var(--gold);margin-bottom:16px">
            riepilogo prenotazione</div>
          <div class="det-row">
        ...
        <div class="form-card">
          <form method="POST" action="prenota.php?s=<?php echo urlencode($slug) ?>">
            ...
            <div class="riga">
              ...
              <div class="campo">
                <label>cognome *</label>
                <input type="text" name="_cognome" value="<?php echo htmlspecialchars($_POST['_cognome'] ?? '') ?>"
                  required>
              </div>
            </div>
            <div class="riga">
              ...
              <select name="servizio_id" required>
                ...
                foreach ($servizi as $s):
                  ...
                  <option value="<?php echo $s['id'] ?>" <?php echo ($_POST['servizio_id'] ?? '') == $s['id'] ? 'selected' : '' ?>>
                    <?php echo htmlspecialchars($s['nome']) ?> — <?php echo $s['durata_minuti'] ?>min —
                    €<?php echo number_format($s['prezzo'], 2, ',', '.') ?>
                  </option>
                <?php endforeach;
                if ($cat_corrente)
                  echo '</optgroup>'; ?>
            ...
            <div class="riga">
              <div class="campo">
                ...
                <input type="date" name="_data" value="<?php echo htmlspecialchars($_POST['_data'] ?? '') ?>"
                  min="<?php echo date('Y-m-d') ?>" required>
              </div>
              <div class="campo">
                <label>ora preferita</label>
                <input type="time" name="_ora" value="<?php echo htmlspecialchars($_POST['_ora'] ?? '') ?>" min="08:00"
                  max="20:00">
            ...
            <div class="campo">
              ...
              <textarea name="note" rows="3"
                placeholder="allergie, preferenze, richieste particolari…"><?php echo htmlspecialchars($_POST['note'] ?? '') ?></textarea>
            </div>
